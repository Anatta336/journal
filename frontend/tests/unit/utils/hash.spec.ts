...
describe("calculateEntryHash", () => {
    it("should return different hash when tags are added", async () => {
        ...
        const hashWithTags = await calculateEntryHash(content, {
            tags: ["work"],
        });
    ...
    it("should return same hash for undefined tags and empty tags array", async () => {
        ...
        const hashUndefinedExplicit = await calculateEntryHash(content, {
            tags: undefined,
        });
    ...
    it("should return same hash regardless of tag order", async () => {
        const content = "Test content";
        const hash1 = await calculateEntryHash(content, {
            tags: ["work", "personal", "journal"],
        });
        const hash2 = await calculateEntryHash(content, {
            tags: ["journal", "work", "personal"],
        });
        const hash3 = await calculateEntryHash(content, {
