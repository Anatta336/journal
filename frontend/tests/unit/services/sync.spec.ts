...
describe('forceRefresh', () => {
    ...
    it('should throw error if offline', async () => {
        ...
        await expect(forceRefresh()).rejects.toThrow('Cannot force refresh while offline')
    })
    ...
    it('should delete local entries not in server manifest', async () => {
        ...
        const serverManifest = [{ id: 'server-entry-1', lastUpdated: '2024-01-01T00:00:00Z' }]
        const serverEntry = {
            id: 'server-entry-1',
        ...
        vi.spyOn(db, 'getAllEntriesIncludingTrashed').mockResolvedValue(mockLocalEntries)
        const hardDeleteSpy = vi.spyOn(db, 'hardDeleteEntry').mockResolvedValue()
    ...
    it('should overwrite local entries with server data', async () => {
        ...
        const serverManifest = [{ id: 'entry-1', lastUpdated: '2024-01-02T00:00:00Z' }]
        const serverEntry = {
            id: 'entry-1',
    ...
    it('should update sync state after completion', async () => {
        ...
        vi.spyOn(db, 'hardDeleteEntry').mockResolvedValue()
        vi.spyOn(db, 'saveEntry').mockResolvedValue()
        const setSyncStateSpy = vi.spyOn(db, 'setSyncState').mockResolvedValue()
        ...
        mockFetch.mockImplementation((url: string) => {
            ...
            if (url.includes('/sync/status')) {
                return Promise.resolve({
                    ok: true,
                    json: () => Promise.resolve({ globalHash: 'new-global-hash' }),
                })
            }
            return Promise.resolve({ ok: false })
        })
