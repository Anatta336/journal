...
test.describe("Force Refresh", () => {
    ...
    test("shows confirmation dialog when Force Refresh is clicked", async ({
        page,
    }) => {
        await page.goto("/settings");
        await waitForSync(page);
        ...
        expect(dialogMessage).toContain(
            "Force Refresh will overwrite all local changes",
        );
    ...
    test("does not proceed if confirmation is cancelled", async ({ page }) => {
        ...
        await expect(
            page.getByTestId("force-refresh-result"),
        ).not.toBeVisible();
    });
    ...
    test("overwrites locally modified entries with server version", async ({
        page,
    }) => {
    ...
    test("removes local-only entries not present on server", async ({
        page,
    }) => {
    ...
    test("fetches entries from server that are not present locally", async ({
        page,
    }) => {
    ...
    test("button is disabled while force refresh is in progress", async ({
        page,
    }) => {
    ...
    test("button is disabled while regular sync is in progress", async ({
        page,
    }) => {
        await createEntry("Test entry");
