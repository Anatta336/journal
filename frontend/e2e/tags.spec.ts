...
async function createEntry(content: string, tags?: string[]): Promise<EntryResponse> {
...
test.describe('Tags', () => {
    ...
    test.describe('Creating Entries with Tags', () => {
        test('can create a new entry with tags', async ({ page }) => {
            ...
            const entryItem = page.getByTestId('entries-list').locator('li').first()
        ...
        test('can add multiple tags to an entry', async ({ page }) => {
            ...
            await expect(page.getByTestId('selected-tag-work')).toBeVisible()
            await expect(page.getByTestId('selected-tag-important')).toBeVisible()
        ...
        test('validates tag format - only alphanumeric and hyphens', async ({ page }) => {
            await page.goto('/entries/new')
            ...
            await expect(page.getByTestId('selected-tag-invalidtag')).toBeVisible()
        })
        ...
        test('can remove a tag before saving', async ({ page }) => {
            ...
            await expect(page.getByTestId('selected-tag-temporary')).toBeVisible()

            await page.getByTestId('remove-tag-temporary').click()

            await expect(page.getByTestId('selected-tag-temporary')).not.toBeVisible()
        })
    })
    ...
    test.describe('Editing Entries with Tags', () => {
        test('loads existing tags when editing an entry', async ({ page }) => {
            ...
            await expect(page.getByTestId('selected-tag-existing-tag')).toBeVisible()
        })
        ...
        test('can remove tags from an existing entry', async ({ page }) => {
            const entry = await createEntry('Entry with tag to remove', ['remove-me'])

            await page.goto(`/entries/${entry.id}`)
            await page.reload()

            await expect(page.getByTestId('selected-tag-remove-me')).toBeVisible()
    ...
    test.describe('Tag Autocomplete', () => {
        test('shows existing tags in autocomplete dropdown', async ({ page }) => {
            ...
            await expect(page.getByTestId('tag-option-existing-tag')).toBeVisible()
        })
        ...
        test('filters autocomplete based on input', async ({ page }) => {
            ...
            await expect(page.getByTestId('tag-option-personal')).not.toBeVisible()
        })
        ...
        test('selecting from autocomplete adds the tag', async ({ page }) => {
            ...
            await expect(page.getByTestId('selected-tag-autocomplete-tag')).toBeVisible()
        })

        test('uses existing casing when adding tag that matches case-insensitively', async ({ page }) => {
            ...
            await expect(page.getByTestId('selected-tag-WorkTag')).toBeVisible()
        })
    })
    ...
    test.describe('Filtering by Tags', () => {
        ...
        test('opens filter panel when clicking Filters button', async ({ page }) => {
            ...
            await expect(page.getByTestId('filter-tag-panel-test')).toBeVisible()
        })
        ...
        test('shows filter count when filters are active', async ({ page }) => {
            ...
            await expect(page.getByTestId('filter-toggle-btn')).toContainText('Filters (1)')
        })
        ...
        test('clears all filters when clicking remove all filters', async ({ page }) => {
        ...
        test('shows message when no entries match filters', async ({ page }) => {
            ...
            await expect(page.getByText('No entries match the selected filters')).toBeVisible()
        })
    })

    test.describe('Tags Display on Entry List', () => {
        test('displays tags as badges on entry cards', async ({ page }) => {
            await createEntry('Entry with tags', ['badge-tag-1', 'badge-tag-2'])

            await page.goto('/entries')
            await page.reload()

            await expect(page.getByTestId('entry-tag-badge-tag-1')).toBeVisible()
            await expect(page.getByTestId('entry-tag-badge-tag-2')).toBeVisible()
        })
    })
})
