...
test.describe('Journal Editor', () => {
    ...
    test('typing * displays * to user but stores as \\* in markdown', async ({ page }) => {
        ...
        const markdown = await page.evaluate(() => {
            const editorComponent = (document.querySelector('.journal-editor') as HTMLElement & {
                __vueParentComponent?: { exposed?: { getMarkdown?: () => string } }
    ...
    test('typing * in code block stores literal *', async ({ page }) => {
        ...
        const codeBlockElement = editor.locator('pre')
        await expect(codeBlockElement).toContainText('function multiply(a, b) { return a * b }')
    })
...
test.describe('Integration - Complete User Workflows', () => {
    test('writing a journal entry with mixed formatting', async ({ page }) => {
        ...
        await expect(editor).toContainText('Today I learned about Tiptap, a headless editor framework.')
    })
    ...
    test('using toolbar to format text preserves markdown structure', async ({ page }) => {
        ...
        await expect(editor.locator('code').first()).toContainText('inline code')
    ...
    test('typing special characters displays them correctly', async ({ page }) => {
        ...
        await expect(editor).toContainText('I want to say *wow* and _amazing_!')
    ...
    test('complete workflow with all formatting types', async ({ page }) => {
        ...
        await expect(editor.locator('pre')).toContainText('console.log("Hello!");')
    })
})
