...
The plan should give you enough information, but if you need more context use a sub-agent to review other files, especially `requirements.md` in ${fileDirname} which should specify the functionality needed which this plan is implementing.

As you implement the plan, ensure that you create or update automated tests to verify that each part of the implementation is correct. Use the testing tools available to you to run these tests frequently during development.
