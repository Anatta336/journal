...
The backend also maintains a SQLite database at `data/journal.db` (see `backend/src/db.ts`). It stores the password hash, bearer tokens used for authentication, and OAuth client/token data for MCP access. The `sqlite-vec` extension is loaded at startup to support future vector search.

**Authentication:** The backend requires the `AUTH_PASSWORD` environment variable to be set on first run (or whenever the password changes). All API endpoints except `POST /auth/login` and MCP/OAuth endpoints require a valid `Authorization: Bearer <token>` header. Tokens expire after `TOKEN_EXPIRY_DAYS` days (default: `10`). See `backend/.env.example` for configuration reference. Tokens are also marked as invalid if the user chooses to logout. There is a "Invalidate all tokens" option that the user can trigger to mark every token as invalid.
...
The backend includes an MCP (Model Context Protocol) server at `/mcp` for read-only access to journal entries. Implementation is in `backend/src/mcp/`. The server provides three tools: `list_tags`, `list_entries_by_tag`, and `get_entry`. It uses Streamable HTTP transport (stateless mode) and OAuth 2.0 with PKCE for authentication. The `MCP_BASE_URL` environment variable configures URLs in OAuth metadata (defaults to `http://localhost:3013`).
