...
- **WHEN** `GET /api/sync/manifest` is called
...
- **WHEN** `POST /api/sync/batch` is called with an entry in `updates`
- **THEN** the entry is saved to disk and its hash is stored in frontmatter

#### Scenario: Batch deletion moves entries to trash
- **WHEN** `POST /api/sync/batch` is called with an entry id in `deletions`
